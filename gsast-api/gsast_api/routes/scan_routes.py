...
def start_scan():
    ...
    scanners = scan_config.scanners or scanner_service.get_default_scanners()
    ...
    try:
        is_valid, error_msg = scanner_service.validate(scanners, rule_files=rule_files, rules_dir=rules_dir)
        if not is_valid:
            if rules_dir:
                shutil.rmtree(rules_dir)
